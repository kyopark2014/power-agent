...
# Configuration
project_name = "power-trade"  # Knowledge Base name; matches update_config.py / utils.py
region = "us-west-2"
...
def create_cloudfront_distribution(s3_bucket_name: str) -> Dict[str, str]:
    ...
    try:
        ...
        for oai in oai_list.get("CloudFrontOriginAccessIdentityList", {}).get("Items", []):
            if f"{oai_comment}" in oai.get("Comment", ""):
                oai_id = oai["Id"]
...
def main():
    ...
    try:
        ...
        # 3. Create IAM roles and AgentCore Web Search gateway
